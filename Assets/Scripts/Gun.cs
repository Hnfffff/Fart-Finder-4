...
public class Gun : MonoBehaviour
{
    ...
    public float bulletSpread = 0.1f; //how far the bullets spread
    ...
    public int maxReserveAmmo = 300; //how much ammo you can hold in reserve
    ...
    private bool onCooldown = false; //if the firing is on cooldown

    public string gunCategory; //string for the gun type

    public float bulletDamage = 1f; //how much damage the bullet does

    public Transform gunPos; //Where the bullets are coming out from

    public Transform camTransform; //Angle of the Camera

    public TMPro.TMP_Text reloadText; //text that holds the ammo and stuff

    public Vector3 spread; //the spread
    ...
    void Start() //Called when the scene/Program starts
    {
        ...
        reserveAmmo = maxReserveAmmo - magazineAmmunation; //reserveammo is equal to the max reserve ammo minus the ammo in the magazine
        reloadText.text = $"Ammo: {magazineAmmunation}/ {reserveAmmo}"; //set the reload text to show the ammo in magazine and in reserve
    }
    ...
    void Update()//Called Once Per Frame
    {
        ...
        if(magazineAmmunation == 0 && !isReloading) //if no ammo in magazine and not already reloading
        {
            StartCoroutine (Reloading()); //start reloating coroutine
        }
    }
    private void Shoot() //Shoots a bullet
    {
        spread = camTransform.transform.forward; //spread is equal to the forward direction of camTransform
        reloadText.text = $"Ammo: {magazineAmmunation}/ {reserveAmmo}"; //set reload text to the magazine ammo and the reserve ammo
        Debug.Log(magazineAmmunation); //flag
        ...
        {
            spread = spread + camTransform.transform.TransformDirection(new Vector3(Random.Range(-bulletSpread, bulletSpread), Random.Range(-bulletSpread, bulletSpread)));//spread = the spread + the a direction consisting of a random range of the bullet spread
            ...
            {
                Destroy(Instantiate(bulletDecal, bulletHitPos.point + bulletHitPos.normal * 0.0001f, Quaternion.FromToRotation(Vector3.up, bulletHitPos.normal)), 10f); //Bit weird how it looks, but pretty much just creates an object (bulletDecal) at the hit postition of the raycast + the normal of it + a small number so there is no z fighting, rotates it to Vector3.up, and the normal of the hit object (the way that the face of the model is pointing), and then deleting it after 10 seconds so it doesnt destroy the frames if you decide to spray your gun a lot
                //Debug.Log($"Hit Object!"); //Debugging
            }
        }
        StartCoroutine(ShotCooldown()); //start cooldown coroutine
    }
    private void Reload() //reload function
    {
        if(reserveAmmo >= magazineSize) //if the reserveammo is bigger than magazinesize (making sure you have enough ammo to reload)
        {
           reserveAmmo -= magazineSize; //remove an amount equal to the magazinesize from the reserve
           magazineAmmunation = magazineSize;//the amount of ammo in the magazine is equal to the magazine size
        }
        else //if theres less than a magazine left in reserve
        {
            magazineAmmunation = reserveAmmo; //the magazines ammo is equal to whats left
            reserveAmmo = 0; //reserve = 0
        }
    }
    IEnumerator Reloading() //reloading enumerator
    {
        Debug.Log("Started Reloading");//flag
    ...
    IEnumerator ShotCooldown()//cooldown enumerator
    {
